# Python Problem Solving

A structured collection of Python solutions focused on developing strong problem-solving skills, algorithmic thinking, and coding intuition.

This repository is built through consistent practice of programming problems, with emphasis on:

* Pattern Recognition
* Logical Thinking
* Mathematical Reasoning
* Time & Space Complexity Analysis
* Code Optimization
* Clean and Readable Implementations

---

## Structure

```text
Python-Problem-Solving
│
├── Numbers
├── NumberSystems
├── Arrays
├── Strings
├── Sorting
│
├── README.md
└── .gitignore
```

---

## Topics Covered

### Numbers

* Prime Numbers
* Prime Factors
* Armstrong Numbers
* Strong Numbers
* Perfect Numbers
* Abundant & Deficient Numbers
* Harshad Numbers
* Automorphic Numbers
* Factorial
* Fibonacci Series
* GCD & LCM
* Fraction Operations
* Digital Root
* Number Reversal
* Digit Manipulation
* Mathematical Series
* Permutations & Combinations

### Number Systems

* Decimal to Binary
* Binary to Decimal
* Octal Conversions
* Hexadecimal Conversions
* Bit Manipulation Basics

### Arrays

* Traversal
* Searching
* Sorting
* Frequency Problems
* Prefix/Suffix Techniques
* Two-Pointer Problems

### Strings

* String Manipulation
* Palindrome Problems
* Frequency Counting
* Pattern Matching
* Character Operations

### Sorting

* Bubble Sort
* Selection Sort
* Insertion Sort
* Merge Sort
* Quick Sort
* Sorting-Based Problem Solving

---

## Learning Approach

Each problem is solved with the following mindset:

1. Understand the underlying pattern.
2. Build intuition before coding.
3. Write a working solution.
4. Analyze Time Complexity.
5. Analyze Space Complexity.
6. Optimize wherever possible.
7. Maintain clean and readable code.

The objective is not only to solve problems but also to understand *why* a solution works and *when* a particular pattern should be applied.

---

## Key Concepts Practiced

* Digit Extraction (`%10`, `//10`)
* Factor Pair Optimization (`√n`)
* Prime Number Techniques
* Mathematical Formulas
* Euclidean Algorithm
* Recursion Fundamentals
* Pattern Recognition
* Complexity Analysis
* Optimization Strategies

---

## Goal

To strengthen Python programming fundamentals and build a solid foundation in problem solving, data structures, algorithms, aptitude coding, and technical interview preparation.

This repository serves as a personal learning journey and a continuously growing collection of optimized solutions.
